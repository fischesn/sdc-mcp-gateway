...
def _row_for_summary(path: Path, summary: dict[str, Any]) -> dict[str, Any]:
    ...
    row: dict[str, Any] = {
        ...
        "active_alarm_count_max": summary.get("active_alarm_count_max"),
        "active_alarm_seen_any": summary.get("active_alarm_seen_any"),
...
def summarize_benchmarks(config: BenchmarkSummaryConfig) -> dict[str, Any]:
    ...
    aggregate = {
        ...
        "active_alarm_count_last_values": sorted({int(summary.get("active_alarm_count_last") or 0) for summary in summaries}),
        "active_alarm_count_max_values": sorted({int(summary.get("active_alarm_count_max") or 0) for summary in summaries}),
        "active_alarm_seen_any": any(summary.get("active_alarm_seen_any") is True for summary in summaries),
